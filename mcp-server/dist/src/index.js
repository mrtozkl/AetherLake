    ...
    try {
        switch (request.params.name) {
            case "get_platform_status": {
                ...
                const statuses = res.body.items.map(pod => ({
            ...
            case "get_service_logs": {
                const { service, lines = 100 } = request.params.arguments;
                // First find a pod matching the service string in its name
                const podList = await k8sApi.listNamespacedPod(NAMESPACE);
                const targetPod = podList.body.items.find(p => p.metadata?.name?.includes(service));
                if (!targetPod || !targetPod.metadata?.name) {
                    return { content: [{ type: "text", text: `No pod found matching service: ${service}` }] };
                }
                const logs = await k8sApi.readNamespacedPodLog(targetPod.metadata.name, NAMESPACE, undefined, undefined, undefined, undefined, undefined, undefined, lines);
                return { content: [{ type: "text", text: logs.body }] };
            }
            case "restart_service": {
                const { serviceLabel } = request.params.arguments;
                const res = await k8sApi.deleteCollectionNamespacedPod(NAMESPACE, undefined, undefined, undefined, undefined, undefined, serviceLabel);
            ...
            case "query_trino": {
                ...
                while (nextUri) {
                    ...
                    if (nextRes.data?.columns && columns.length === 0)
                        columns = nextRes.data.columns;
            ...
            case "trigger_airflow_dag": {
                try {
                    const { dag_id } = request.params.arguments;
